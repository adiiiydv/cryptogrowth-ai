...
```
    ...
    // BUG FIX 5: use consistent field — prefer `market`, fall back to `coindcx_name`
    const data = tickerRes.data.find(m => m.market === marketName || m.coindcx_name === marketName);
    ...
    // BUG FIX 3: treat exactQty as a number check, not truthiness, to handle 0 edge cases
    const qty = (exactQty !== null && exactQty > 0)
    ...
    if (retry > 0) return new Promise(resolve =>
        setTimeout(() => resolve(executeOrder(side, binanceSymbol, amount, exactQty, retry - 1)), 3000)
    );
...
// BUG FIX 4: snapshot the hour once at the top so mid-loop boundary changes don’t cause issues
...
try {
    ...
    // BUG FIX 1: guard against undefined locked_balance before subtraction
...
for (let i = activeTrades.length - 1; i >= 0; i--) {
    ...
    // BUG FIX 5 (continued): consistent field name in exit loop too
    const ticker = allTickers.find(m => m.market === t.marketId || m.coindcx_name === t.marketId);
    if (!ticker) continue;
    ...
    // BUG FIX 2: t.stop is an absolute price delta (ATR * 1.5), so convert to % for comparison
    const stopPct = (t.stop / t.entry) * 100;

    const shouldTakeProfit = gain > (1.2 + FEES) && drop > 0.3;
    const shouldStopLoss = gain < -stopPct;
    ...
    if (shouldTakeProfit || shouldStopLoss) {
        ...
        if (sold) {
            ...
            saveData();
            botLog(`📊 Trade closed | Gain: ${gain.toFixed(2)}% | Streak: ${lossStreak}`);
        }
    }
}
    ...
    if (score === 3 && tradesThisHour < TARGET_TRADES_PER_HOUR && lastKnownBal > 5.0) {
        ...
        if (bought) {
            activeTrades.push({
                ...
                // store stop as absolute price delta; stopPct conversion happens at exit
...
botLog(`✅ APEX PRO v14.5 FIXED | PORT ${PORT}`);
runScanner();
