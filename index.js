...
app.get('/', (req, res) => res.json({ status: "Online", active: activeTrades.length, balance: lastKnownBal }));
...
// --- 6. SCANNER LOGIC (WITH VISUAL DEBUGGER) ---
const runScanner = async () => {
    ...
    for (const coin of WATCHLIST) {
        ...
        // 🔥 VISUAL DEBUGGER: SEE THE MATH IN RENDER LOGS
        let score = 0;
        let reasons = [];
        if (rsi < 65) { score++; reasons.push("RSI_LOW"); }
        if (ema9 > ema21) { score++; reasons.push("EMA_BULL"); }
        if (closes[closes.length - 1] > closes[closes.length - 2]) { score++; reasons.push("MOMENTUM"); }

        // Log the scan status for every coin
        console.log(`🔍 SCANNING ${coin} | RSI: ${rsi.toFixed(1)} | Score: ${score}/3 | Reasons: [${reasons.join(", ")}]`);

        if (score >= 2 && lastKnownBal > 5) {
            const tradeAmt = Math.min(lastKnownBal * 0.35, lastKnownBal - 0.1).toFixed(2);
            console.log(`🚀 SIGNAL DETECTED: Entering ${coin} with ${tradeAmt} USDT...`);
...
async function checkExits(t, idx) {
    try {
        ...
        if ((gain > 0.8 && drop > trail) || gain < -(stopPercent + 0.25)) {
            console.log(`🚪 EXIT SIGNAL: ${t.symbol} | Gain: ${gain.toFixed(2)}% | Drop: ${drop.toFixed(2)}%`);
